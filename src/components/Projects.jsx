          ...
          {projects.map((project, index) => (
            ...
            >
              <motion.div
                ...
                className="group relative h-full rounded-[2.5rem] bg-dark-800/40 backdrop-blur-xl border border-white/10 hover:border-white/30 overflow-hidden flex flex-col z-10 shadow-2xl transition-all duration-300"
              >
                {/* Advanced Animated Moving Gradient Background */}
                <div className={`absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-700 z-0 bg-gradient-to-br ${project.color}`}></div>

                {/* Animated Light Orb */}
                <motion.div
                  animate={{ x: [0, 30, -10, 0], y: [0, -30, 10, 0] }}
                  transition={{ duration: 6, repeat: Infinity, ease: "linear" }}
                  className={`absolute -top-10 -right-10 w-40 h-40 bg-gradient-to-br ${project.color} rounded-full blur-[60px] opacity-20 group-hover:opacity-80 transition-opacity duration-500 z-0`}
                ></motion.div>
                ...
                <div className="relative z-10 p-8 flex flex-col flex-grow">
                  ...
                  <div className="flex items-start justify-between mb-10">
                    <motion.div
                      ...
                      className={`w-16 h-16 rounded-[1.2rem] bg-gradient-to-br ${project.color} flex items-center justify-center text-white shadow-[0_0_20px_rgba(0,0,0,0.3)] cursor-pointer relative overflow-hidden`}
                      onClick={() => project.isBundle && setShowModal(true)}
                    >
                      {/* Shine effect inside icon */}
                      <div className="absolute inset-x-0 -top-full h-full bg-gradient-to-b from-transparent via-white/40 to-transparent group-hover:top-full transition-all duration-700 ease-in-out"></div>
                      <span className="w-8 h-8 flex items-center justify-center relative z-10 drop-shadow-md">
                        {project.icon}
                      </span>
                    </motion.div>
                    ...
                    {project.isBundle && (
                      <motion.div
                        ...
                        className="bg-dark-900/80 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/10 text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2 shadow-xl"
                      >
                        <span className={`w-2 h-2 rounded-full bg-gradient-to-r ${project.color} animate-pulse`}></span>
                        Bundle
                      </motion.div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col flex-grow">
                    <motion.h3
                      className="text-2xl font-black text-white mb-3 tracking-tight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-slate-400 transition-all duration-300"
                    >
                      {project.title}
                    </motion.h3>
                    <p className="text-slate-400 text-sm mb-8 flex-grow leading-relaxed font-medium group-hover:translate-x-1 group-hover:text-slate-300 transition-all duration-300">
                      {project.description}
                    </p>

                    {/* Tech Stack staggered animation on hover */}
                    <div className="flex flex-wrap gap-2 mb-8">
                      {project.tech.map((t) => (
                        <motion.div
                          ...
                          className="flex items-center gap-2 px-3 py-1.5 bg-dark-900/60 rounded-xl border border-white/5 hover:border-white/30 transition-colors duration-300 shadow-sm"
                        >
                          <span className="text-base">{getTechIcon(t)}</span>
                          <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">{t}</span>
                        </motion.div>
                      ))}
                    </div>

                    {/* Footer Links */}
                    <div className="flex items-center justify-between mt-auto pt-6 border-t border-white/10 relative overflow-hidden">
                      {/* Animated border bottom line */}
                      <div className={`absolute bottom-0 left-0 h-[2px] w-0 bg-gradient-to-r ${project.color} group-hover:w-full transition-all duration-700 ease-out`}></div>

                      {project.isBundle ? (
                        <button
                          onClick={() => setShowModal(true)}
                          className="flex items-center gap-3 text-sm font-bold text-white group/link focus:outline-none"
                        >
                          <span className="relative overflow-hidden flex items-center h-6">
                            <span className="flex items-center h-full group-hover/link:-translate-y-full transition-transform duration-300">View Projects</span>
                            <span className={`absolute top-full flex items-center h-full group-hover/link:-translate-y-full transition-transform duration-300 text-transparent bg-clip-text bg-gradient-to-r ${project.color}`}>View Projects</span>
                          </span>
                          <motion.span
                            className="bg-white/10 w-8 h-8 rounded-full flex items-center justify-center text-white"
                        ...
                        >
                          <span className="relative overflow-hidden flex items-center h-6">
                            <span className="flex items-center h-full group-hover/link:-translate-y-full transition-transform duration-300">Live Demo</span>
                            <span className={`absolute top-full flex items-center h-full group-hover/link:-translate-y-full transition-transform duration-300 text-transparent bg-clip-text bg-gradient-to-r ${project.color}`}>Live Demo</span>
                          </span>
                          <motion.span
                            className="bg-white/10 w-8 h-8 rounded-full flex items-center justify-center text-white"
                            whileHover={{ x: 4, y: -4, backgroundColor: "rgba(255,255,255,0.2)" }}
                          >
                      ...
                      <motion.a
                        whileHover={{ scale: 1.15, rotate: 12, backgroundColor: 'rgba(255,255,255,0.15)' }}
                        href={project.githubLink || "https://github.com/amaan2t3"}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-10 h-10 rounded-full bg-dark-900/50 border border-white/10 flex items-center justify-center text-slate-400 hover:text-white transition-all shadow-md group-hover:border-white/30"
